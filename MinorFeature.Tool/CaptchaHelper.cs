...
namespace MinorFeature.Tool
{
    ...
    public static class CaptchaHelper
    {
        ...
        private static void ImageCaptcha()
        {
        ...
        /// <returns>图片流字节</returns>
        public static byte[] GetCaptcha(string captchaText, int width, int height, int lineNum = 1, int lineStrookeWidth = 1)
        {
            ImageCaptcha();
            //创建bitmap位图
            using (SKBitmap image2d = new SKBitmap(width, height, SKColorType.Bgra8888, SKAlphaType.Premul))
            {
                ...
                using (SKCanvas canvas = new SKCanvas(image2d))
                {
                    ...
                    using (SKPaint drawStyle = CreatePaint(SKColors.Black, height))
                    {
                        ...
                        canvas.DrawText(captchaText, 1, height - 1, SKTextAlign.Center, new SKFont(font), paint);
                    }
                    //画随机干扰线
